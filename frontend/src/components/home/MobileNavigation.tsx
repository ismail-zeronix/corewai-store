            ...
            <Link key={href} href={href} aria-current={active ? "page" : undefined}
              ...
              <span className="relative">
                ...
                {label === "Cart" && isHydrated && itemCount > 0 && (
                  <span aria-hidden="true" className="absolute -right-3 -top-1 min-w-4 rounded-full bg-lime px-1 text-center text-[10px] leading-4 font-semibold text-ink">{itemCount > 99 ? "99+" : itemCount}</span>
                )}
              </span>
              {label}
            </Link>
